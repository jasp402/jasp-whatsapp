...
let futoin_hkdf_1   = __importDefault(require("futoin-hkdf"));
let atob_1          = __importDefault(require("atob"));
let axios_1         = __importDefault(require("axios"));
...
let makeOptions       = function (useragentOverride) {
    return ({
...
exports.decryptMedia  = function (message, useragentOverride) {
    return __awaiter(void 0, void 0, void 0, function () {
        let options, haventGottenImageYet, res, error_1, buff;
...
let processUA         = function (userAgent) {
...
let magix             = function (fileData, mediaKeyBase64, mediaType, expectedSize) {
    let encodedHex       = fileData.toString('hex');
    let encodedBytes     = hexToBytes(encodedHex);
    let mediaKeyBytes    = base64ToBytes(mediaKeyBase64);
    let info             = "WhatsApp " + exports.mediaTypes[mediaType.toUpperCase()] + " Keys";
    let hash             = 'sha256';
    let salt             = new Uint8Array(32);
    let expandedSize     = 112;
    let mediaKeyExpanded = futoin_hkdf_1.default(mediaKeyBytes, expandedSize, {
        salt: salt,
        info: info,
        hash: hash
    });
    let iv               = mediaKeyExpanded.slice(0, 16);
    let cipherKey        = mediaKeyExpanded.slice(16, 48);
    let decipher         = crypto_1.default.createDecipheriv('aes-256-cbc', cipherKey, iv);
    let decoded          = decipher.update(encodedBytes);
    let mediaDataBuffer  = expectedSize ? fixPadding(decoded, expectedSize) : decoded;
    return mediaDataBuffer;
};
let fixPadding        = function (data, expectedSize) {
    ...
    if (padding > 0) {
        if ((expectedSize + padding) == data.length) {
            ...
            let arr = new Uint16Array(padding).map(function (b) {
                return padding;
            });
...
let hexToBytes        = function (hexStr) {
    let intArray = [];
...
let base64ToBytes     = function (base64Str) {
    let binaryStr = atob_1.default(base64Str);
